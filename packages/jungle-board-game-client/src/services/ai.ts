...
export function getPossibleMoves(
  ...
  const opponentTurn =
    computerTurn === gameLogic.PlayerSymbol.B
      ? gameLogic.PlayerSymbol.W
      : gameLogic.PlayerSymbol.B;
  for (let row = 0; row < gameLogic.ROWS; row++) {
    for (let col = 0; col < gameLogic.COLS; col++) {
      const piece = board[row][col];

      if (
        piece === gameLogic.PieceName.L ||
        piece === gameLogic.PieceName.R ||
        piece.charAt(0) !== computerTurn
      ) {
        continue;
      }
      ...
      switch (piece.substring(1)) {
        case gameLogic.Animal.Elephant:
          possibleMoves = gameLogic.getElephantPossibleMoves(
            board,
            computerTurn,
            deltaFrom
          );
          ...
          if (possibleMoves.length > 0) {
            for (let deltaTo of possibleMoves) {
              ...
              if (
                pieceTo === gameLogic.PieceName.L ||
                pieceTo === gameLogic.PieceName.R
              ) {
        ...
        case gameLogic.Animal.Lion:
          possibleMoves = gameLogic.getLionPossibleMoves(
            board,
            computerTurn,
            deltaFrom
          );
        ...
        case gameLogic.Animal.Tiger:
          possibleMoves = gameLogic.getTigerPossibleMoves(
            board,
            computerTurn,
            deltaFrom
          );
        ...
        case gameLogic.Animal.Leopard:
          possibleMoves = gameLogic.getLeopardPossibleMoves(
            board,
            computerTurn,
            deltaFrom
          );
        ...
        case gameLogic.Animal.Dog:
          possibleMoves = gameLogic.getDogPossibleMoves(
            board,
            computerTurn,
            deltaFrom
          );
        ...
        case gameLogic.Animal.Wolf:
          possibleMoves = gameLogic.getWolfPossibleMoves(
            board,
            computerTurn,
            deltaFrom
          );
        ...
        case gameLogic.Animal.Cat:
          possibleMoves = gameLogic.getCatPossibleMoves(
            board,
            computerTurn,
            deltaFrom
          );
        ...
        case gameLogic.Animal.Mouse:
          possibleMoves = gameLogic.getMousePossibleMoves(
            board,
            computerTurn,
            deltaFrom
          );
...
export function createComputerMove(
  board: gameLogic.Board,
  computerTurn: string
): gameLogic.BoardDelta[] {
  const { lowLevelMoves, midLevelMoves, highLevelMoves } = getPossibleMoves(
    board,
    computerTurn
  );
  let returnedIndex = 0;
